...
Para **você**: um cartão do **USDT/BRL** com o resumo dos dois timeframes — último fechamento, lado e distância da EMA89 **em porcentagem**, RSI, ADX com DI+/DI−, estrutura, situação dos níveis manuais —, mais os alertas técnicos como etiquetas.
...
Só o **USDT/BRL** tem gráfico embutido, com `BINANCE:USDTBRL` — a mesma fonte primária do relatório. Vem só com a **EMA89** já carregada, a mesma linha que o cartão cita. RSI e ADX ficam de fora de propósito: os dois juntos poluem o embed, e quem quer um deles escolhe na hora, pelo próprio widget. Ele é desenhado pelo TradingView e redesenhado quando o tema muda, porque mora num iframe e o tema dele é escolhido na criação do widget, não por CSS.
